...
class RoutineTest extends AbstractPluginTestCase
{
    ...
    public function testDiffRoutine(): void
    {
        ...
        self::assertFalse($comparator->diffRoutine(
            new Routine('hoge', 'select 1', ['option' => 1]),
            new Routine('fuga', 'select 1', ['option' => 1]),
        ));
        self::assertTrue($comparator->diffRoutine(
            new Routine('hoge', 'select 1', ['option' => 1]),
    ...
    public function testRoutine(): void
    {
        ...
        $schema2->addRoutine($createdRoutine1 = new Routine('created1', 'select 1', $poptions));
        $schema2->addRoutine($createdRoutine2 = new Routine('created2', 'select 1', $foptions));
            ...
            switch (true) {
                case $platform instanceof MySQLPlatform:
                    return [
                        ...
                        'created2' => [
                            <<<'SQL'
                            CREATE FUNCTION created2(arg1 tinyint, arg2 datetime)
                            RETURNS varchar(1)
                            NOT DETERMINISTIC
                            dummy
                            COMMENT 'this is comment'
        ...
        foreach ($expected as $key => $sqls) {
            foreach ($sqls as $sql) {
                $message = "$key failed\n" . implode("\n", array_filter($actual, fn($v) => strpos($v, $key) !== false));
