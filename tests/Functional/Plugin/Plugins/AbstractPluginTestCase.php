...
abstract class AbstractPluginTestCase extends FunctionalTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if ($this->autoskip) {
            ...
            $traits = array_merge(
                ...
                $this->getAllTrait($this->connection->createSchemaManager()),
            );

            $traits = array_filter($traits, fn($t) => ! str_contains($t, 'Abstract') && str_contains($t, "Plugin\\$pluginName"));
    ...
    protected function tearDown(): void
    {
        ...
        (function () {
            if (isset($this->instances['bit'])) {
                unset($this->instancesReverseIndex[spl_object_id($this->instances['bit'])]);
                unset($this->instances['bit']);
            }
        })->bindTo(Type::getTypeRegistry(), Type::getTypeRegistry())();

        parent::tearDown();
    }

    private function getAllTrait($class): array
    {
    ...
    function dropAndCreateSchemaObject(AbstractAsset $object): string
    {
    ...
    public static function assertException($e, $callback): void
    {
        if (is_string($e)) {
            $e = new \Exception($e);
        }
