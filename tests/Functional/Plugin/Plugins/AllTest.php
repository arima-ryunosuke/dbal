...
class AllTest extends AbstractPluginTestCase
{
    ...
    public function test_mysql(): void
    {
        ...
        array_map(fn($sql) => $con1->executeStatement($sql), array_merge($dropSqls, [
            ...
            ) Comment='comment_from' CHARSET=utf8 COLLATE='utf8_general_ci' ROW_FORMAT=COMPRESSED",
            ...
            "CREATE SQL SECURITY DEFINER VIEW ExampleView AS SELECT id, seq, content FROM ExampleTable WITH LOCAL CHECK OPTION",
        ...
        array_map(fn($sql) => $con2->executeStatement($sql), array_merge($dropSqls, [
            "CREATE TABLE NewTable (
              id INT(10) UNSIGNED NOT NULL,
              PRIMARY KEY (id)
            ) Comment='new_table' CHARSET=utf8 COLLATE='utf8_bin' ENGINE=InnoDB",
            ...
            "CREATE TABLE ExampleTable (
              ...
              content LONGTEXT DEFAULT (_utf8mb4'I\'s') NOT NULL COLLATE utf8_bin,
              bindata LONGBLOB NOT NULL,
              bitdata BIT(2) DEFAULT b'0' NOT NULL,
              enumdata ENUM('Y', 'N', 'D') NOT NULL,
              setdata ENUM('Y', 'N', 'D') NOT NULL,
            ...
            ) Comment='comment_to' CHARSET=utf8 COLLATE='utf8_bin' ROW_FORMAT=DYNAMIC",
            ...
            "CREATE SQL SECURITY INVOKER VIEW ExampleView AS SELECT id, seq, name, content FROM ExampleTable WITH CASCADED CHECK OPTION",
        ...
        $expecteds = [
            ...
            // contains new table
            "CREATE TABLE NewTable (id INT UNSIGNED NOT NULL, PRIMARY KEY (id)) DEFAULT CHARACTER SET utf8 COLLATE `utf8_bin` ENGINE = InnoDB COMMENT = 'new_table' ROW_FORMAT = Dynamic",
            "CREATE DEFINER=`user2`@`%` TRIGGER NewTable_trg AFTER INSERT ON NewTable FOR EACH ROW",
            ...
            "CHANGE setdata setdata ENUM('Y', 'N', 'D') NOT NULL AFTER enumdata",
            "CHANGE latlng latlng geometry NOT NULL AFTER setdata",
            '-- type: integer => string',
            '-- notnull: false => true',
            ...
            '-- collation: "utf8_general_ci" => "utf8_bin"',
            '-- comment: "comment_from" => "comment_to"',
            '-- row_format: "Compressed" => "Dynamic"',
            "COLLATE `utf8_bin` COMMENT = 'comment_to' ROW_FORMAT = Dynamic",
            ...
            // contains alter view
            '-- sql: "',
            '-- options.securityType: "DEFINER" => "INVOKER"',
            '-- options.checkOption: "LOCAL" => "CASCADED"',
            "SQL SECURITY INVOKER VIEW ExampleView",
        ...
        $con1->getDatabasePlatform()->enableAutoType(false);
        $con2->getDatabasePlatform()->enableAutoType(false);
    }
    ...
    public function test_postgres(): void
    {
        ...
        $con2->executeStatement(<<<DDL
            ...
            CREATE VIEW ExampleView AS SELECT id, seq FROM ExampleTable;
