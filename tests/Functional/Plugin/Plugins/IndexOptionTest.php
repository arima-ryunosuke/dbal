...
class IndexOptionTest extends AbstractPluginTestCase
{
    ...
    public function testImplicitFlag(): void
    {
        $table = new Table('foo', [
            new Column('primary', Type::getType('integer')),
            new Column('secondary', Type::getType('string')),
            new Column('tertiary', Type::getType('string')),
        ], [
            new Index('PRIMARY', ['primary'], true, true),
            new Index('uniq', ['secondary'], true),
        ], [], [
            new ForeignKeyConstraint(['tertiary'], 'dummy', ['dummy']),
        ]);

        $indexes = $table->getIndexes();

        // explicit
        foreach (['primary', 'uniq'] as $name) {
            self::assertFalse($indexes[$name]->hasFlag('implicit'));
            unset($indexes[$name]);
        }

        // implicit
        foreach ($indexes as $index) {
            self::assertTrue($index->hasFlag('implicit'));
        }
    }
