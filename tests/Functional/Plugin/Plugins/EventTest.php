...
class EventTest extends AbstractPluginTestCase
{
    ...
    public function testDiffEvent(): void
    {
        ...
        self::assertFalse($comparator->diffEvent(
            new Event('hoge', 'select 1', ['option' => 1]),
            new Event('fuga', 'select 1', ['option' => 1]),
        ));
        self::assertTrue($comparator->diffEvent(
            new Event('hoge', 'select 1', ['option' => 1]),
    ...
    public function testEvent(): void
    {
        ...
        $schema2->addEvent($createdEvent = new Event('created', 'select 1', $defoptions));
    ...
    public static function provideInterval()
    {
        return [
            'year'          => [
                'field'    => 'year',
     ...
     */
    public function testEventInterval($field, $value, $exptected): void
    {
        ...
        $events = $this->connection->createSchemaManager()->listEvents();
        self::assertEquals($exptected, $events["test_e{$this->dataName()}"]->getOption('interval'));
    }
}
