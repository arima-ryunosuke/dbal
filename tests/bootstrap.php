...
flock($lock, LOCK_EX);
Doctrine\DBAL\Plugin\Pluggable::generateAll();
flock($lock, LOCK_UN);
fclose($lock);
...
$params = (function ($test_dsn) {
    ...
    $parts['driver'] = "pdo_" . $parts['scheme'];
    ...
    foreach (['db', 'tmpdb'] as $prefix) {
        $GLOBALS += [
            "{$prefix}_driver"   => $parts['driver'],
            "{$prefix}_host"     => $parts['host'],
            "{$prefix}_port"     => $parts['port'],
            "{$prefix}_user"     => $parts['user'],
            "{$prefix}_password" => $parts['password'],
            "{$prefix}_dbname"   => $parts['dbname'],
        ];
