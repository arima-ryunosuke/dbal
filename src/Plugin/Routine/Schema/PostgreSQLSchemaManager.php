...
trait PostgreSQLSchemaManager
{
    protected function _getPortableRoutineDefinition(array $routine): Routine
    {
        ...
        $routine['parameters'] = $this->_getPortableRoutineParameter(json_decode($routine['parameters'], true));

        $routine['name'] = $routine['schema_name'] . '.' . $routine['name'];
        unset($routine['schema_name']);
    ...
    protected function selectRoutines(string $databaseName): Result
    {
        $sql = <<<SQL
            SELECT
                ...
                MIN(r.routine_schema)     AS "schema_name",
